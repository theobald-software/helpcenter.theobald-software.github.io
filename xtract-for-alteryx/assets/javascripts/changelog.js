...
document.addEventListener('DOMContentLoaded', () => {
	...
	const renderRow = (item) => {
		const rows = item.Changes.map(change => {
		  ...
		  ${change.Message} ${change.ReleaseNote ? `<br> <button class="show-more" data-release-note="${encodeURIComponent(change.ReleaseNote)}" data-product-name="${change.Product}" data-version="${item.Version}" style="cursor: pointer; color: #ED1A33;">( Release Notes)</button>` : ''}</td>
        `;
			return row;
		});
		return rows;
