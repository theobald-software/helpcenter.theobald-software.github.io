    ...
    const renderRow = (item) => {
      const rows = item.Changes.map(change => {
        ...
        row.innerHTML = `
		  ...
		  ${change.Message} ${change.ReleaseNote ? `<br> <button class="show-more" data-release-note="${encodeURIComponent(change.ReleaseNote)}" style="cursor: pointer; color: #22A285;">( Release Notes)</button>` : ''}</td>
        `;
        return row;
      });
      return rows;
