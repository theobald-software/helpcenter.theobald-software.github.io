    ...
    const renderRow = (item) => {
      const rows = item.Changes.map(change => {
        ...
        row.innerHTML = `
		  ...
		  ${change.Message} ${change.ReleaseNote ? `<br> <button class="show-more" data-release-note="${encodeURIComponent(change.ReleaseNote)}" style="cursor: pointer; color: #0076BE;">( Release Notes)</button>` : ''}</td>
        `;
        return row;
      });
      return rows;
